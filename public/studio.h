...
struct mstudioseqdesc_t
{
	...
	inline void			setanim( int x, int y, int index ) const
	{
		if ( x >= groupsize[0] )
		{
			x = groupsize[0] - 1;
...
struct mstudioseqdesc_version_36_t
{
	int					szlabelindex;
	inline char * const pszLabel( void ) const { return ((char *)this) + szlabelindex; }
...
inline void Studio_ConvertSeqDescsToNewVersion( studiohdr_t *pStudioHdr )
{
	...
	{
		mstudioseqdesc_version_36_t *pSeqVersion36 = (mstudioseqdesc_version_36_t *)((byte *)pStudioHdr + pStudioHdr->seqindex) + iSeq;
	...
	for ( iSeq = 0; iSeq < pStudioHdr->numseq; iSeq++ )
	{
		...
		int indexOffset = iSeq * (sizeof( mstudioseqdesc_version_36_t ) - sizeof( mstudioseqdesc_t ));
