...
CVertexBuffer::CVertexBuffer( 
	...
	m_bLocked(false), m_VertexCount(vertexCount), 
	m_nBufferSize(vertexSize * vertexCount), // VXP
	m_bDynamic(dynamic)
{
#ifdef RECORDING
	// assign a UID
...
//	return (m_Position + (numVertices * m_VertexSize) ) <= m_nBufferSize;
}
...
void CVertexBuffer::Unlock( int numVerts )
{
	if ( ( m_bLocked ) && ( m_pVB ) )
	{
	//	int nLockOffset = m_Position; // VXP: Should I do this here?
	//	int nBufferSize = numVerts * m_VertexSize;
		...
		/*if ( m_bDynamic )
		{
			D3DDevice()->Unlock( m_pVB, &m_LockData, nBufferSize);
		}
		else
		{
			D3DDevice()->Unlock( m_pVB );
		}*/
