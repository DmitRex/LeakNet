...
//	void ComputeStatsInfo( unsigned int textureIdx, bool isCubeMap );
	void ComputeStatsInfo( unsigned int textureIdx, bool isCubeMap, ImageFormat imageFormat );
...
//	m_Caps.m_SupportsNonPow2Textures = ( caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL ) ? true : false;
	m_Caps.m_SupportsNonPow2Textures =	( !(caps.TextureCaps & D3DPTEXTURECAPS_POW2) && !(caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL) ) ||
										( (caps.TextureCaps & D3DPTEXTURECAPS_POW2) && (caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL) );
...
void CShaderAPIDX8::ComputeStatsInfo( unsigned int textureIdx, bool isCubeMap, ImageFormat imageFormat )
{
	Texture_t &textureData = GetTexture( textureIdx );
...
//			Assert( 0 ); // fixme
		...
		//	GetTexture( textureIdx ).m_SizeTexels += 6 * desc.Width * desc.Height;
			textureData.m_SizeBytes += 6 * ImageLoader::GetMemRequired( desc.Width, desc.Height, imageFormat, false );
...
//			Assert( 0 ); // fixme
		...
		//	GetTexture( textureIdx ).m_SizeTexels += desc.Width * desc.Height;
			textureData.m_SizeBytes += ImageLoader::GetMemRequired( desc.Width, desc.Height, imageFormat, false );
...
//	pTexture->m_MagFilter = D3DTEXF_LINEAR; // VXP
	if( isRenderTarget )
	{
	...
	else
	{
		...
		GetTexture( i ).m_MinFilter = D3DTEXF_LINEAR;
		pTexture->m_MinFilter = pTexture->m_MagFilter = D3DTEXF_LINEAR; // VXP
	}
	GetTexture( i ).m_SwitchNeeded = false;
	GetTexture( i ).m_MipFilter = (GetTexture( i ).m_NumLevels != 1) ? 
		D3DTEXF_LINEAR : D3DTEXF_NONE;
	ComputeStatsInfo( i, isCubeMap, dstImageFormat );
